...
// Deployers
// Secure Cryptographic Deployers loaded from real World ID setup
...
if (!fs.existsSync(deployersPath)) {
  ...
  console.error("   To make the system 'all real proper', you must link your real World ID first.");
  console.error("   Run the initialization script to read your .worldauth.json and generate valid Ethereum identities before generating agents.");
  process.exit(1);
} else {
  deployers = JSON.parse(fs.readFileSync(deployersPath, 'utf8'));
}

const contractAddress = process.env.ERC8004_CONTRACT_ADDRESS || "0x5FbDB2315678afecb367f032d93F642f64180aa3";
...
const agentsData = [
  ...
  { id: 14, handle: "@def_analyst_ivan", name: "Ivan Petrov", cat: "military", affil: "RUSI", m: "M3", cycles: [3, 5, 7], kFocus: "Russian/Iranian weapons systems", b: "Research fellow at the Royal United Services Institute. Former Russian military intelligence analyst. Deep expertise in Iranian-Russian defence cooperation, Shahed drone variants, and ballistic missile programmes.", p: ["technical", "weapons-focused", "analytical"], s: "Focuses on weapons systems and their capabilities. Provides technical specifications. Discusses supply chain and manufacturing implications.", ktier: "shared_base + military_supplement", kfile: "knowledge/military_supplement.json" },
  ...
  { id: 16, handle: "@econ_sophie", name: "Sophie Moreau", cat: "economist", affil: "IEA", m: "M3", cycles: [4, 8], kFocus: "Oil supply disruption, OPEC response", b: "Senior analyst at the International Energy Agency. Tracks global oil supply, OPEC production decisions, and energy market disruption from geopolitical events. Based in Paris.", p: ["data-driven", "supply-focused", "institutional"], s: "Cites IEA data and OPEC production figures. Focuses on supply-side analysis and strategic petroleum reserves.", ktier: "shared_base + economist_supplement", kfile: "knowledge/economist_supplement.json" },
  { id: 17, handle: "@humanitarian_tariq", name: "Tariq Hassan", cat: "humanitarian", affil: "UNHCR", m: "M3", cycles: [4, 6], kFocus: "Displacement data, refugee corridors", b: "UNHCR field officer tracking displacement patterns in the Eastern Mediterranean. Monitors refugee flows, shelter capacity, and cross-border humanitarian access. 7 years of field deployment experience.", p: ["field-focused", "data-aware", "urgency-driven"], s: "Reports displacement numbers and shelter status. Uses UNHCR operational data. Focuses on immediate humanitarian needs.", ktier: "shared_base + humanitarian_supplement", kfile: "knowledge/humanitarian_supplement.json" },
